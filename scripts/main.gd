	...
	if quest_phase == 3:
		...
		if player:
			if sec > 0:
				...
				# Spawn extraction zone at plaza center
				spawn_extraction_pad(Vector3(0.0, 0.05, 0.0))
...
func start_battery_quest_phase() -> void:
	...
	# Spawn 2 batteries on opposite sidewalks
	spawn_battery(Vector3(22.0, 0.5, 22.0))
	spawn_battery(Vector3(-22.0, 0.5, -22.0))
	
	# Spawn Radio Transmitter at plaza center
	spawn_radio(Vector3(2.0, 0.5, -2.0))
